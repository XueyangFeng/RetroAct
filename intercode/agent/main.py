...
import json
import argparse
from intercode_trial import run_trial
...
def get_args():
    ...
    parser.add_argument("--base_model", type=str, help="The path of base model")
...
def main(args) -> None:
    if args.is_resume:
        ...
        # Create the run directory run name为运行历史记录的文件夹
        if not os.path.exists(args.run_name):
            os.makedirs(args.run_name)
        logging_dir = args.run_name

        # initialize environment configs 保存一个env_config的临时变量
    ...
    model = LocalModel(model_dir=args.base_model)
    ...
    while trial_idx < args.num_trials:
        ...
        # run trial
        run_trial(args, trial_log_path, world_log_path, trial_idx, env_configs, args.use_memory, model)

        # update memory if needed  reflexion过程，memory保存在env_configs里面
        if args.use_memory:
            env_configs: List[Dict[str, Any]] = update_memory(trial_log_path, env_configs, model)
