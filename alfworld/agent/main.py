...
def main(args) -> None:
    ...
    model = LocalModel(model_dir="/data/pretrain_dir/Llama-2-13b-chat-hf/snapshots/c2f3ec81aac798ae26dcc57799a994dfbf521496")
    while trial_idx < args.num_trials:
        ...
        # run trial
        run_trial(model, trial_log_path, world_log_path, trial_idx, env_configs, args.use_memory)

        # update memory if needed  reflexion过程，memory保存在env_configs里面
        if args.use_memory:
            env_configs: List[Dict[str, Any]] = update_memory(model, trial_log_path, env_configs)
