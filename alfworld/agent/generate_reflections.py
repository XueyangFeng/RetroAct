...
#memory就是每个trial总结的信息
def update_memory(model: LocalModel, trial_log_path: str, env_configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, env in enumerate(env_configs):
        ...
        if not env['is_success'] and not env['skip']:
            ...
            #reflection: str = get_chat(reflection_query) # type: ignoreprompt=prompt, model=model, temperature=cur_try * 0.2, stop_strs=stop
            reflection: str = model.inference(prompt=reflection_query)
            env_configs[i]['memory'] += [reflection]
            env_configs[i]['reflector_prompt'] += [reflection_query]
